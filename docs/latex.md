...
4. 汇总为多文件 `.tex` 项目 → 编译全书 PDF（失败进入**修复会话**：读文件/改文件/重编译 + 字体工具核对样式）→ 成功后进入**终审会话**逐页核对成品 PDF 并整理（`latex.compile.final_review`，默认开启；转录在 `work/sessions/final_review.jsonl`，WebUI 可见、计成本）→ 生成单文件 `standalone.tex`；交付把整棵 build 树复制到 `out/`，`book.pdf` 只是 `main.pdf` 的别名。终审成功后它的树内修改（章节修订 + 新增顶层 .tex + main.tex 钩子）**回写** `work/chapters/` 与 `work/final_review/` 并记内容指纹（`state.sha256`）；续跑时指纹一致（convert 后没动过）直接沿用、**跳过终审会话**，不一致才重审（旧转录归档 `final_review_prev.jsonl`）——build/ 每次重建，不回写的话每次续跑都要把几十轮终审从头再烧。修复会话转录在 `work/sessions/book_fix.jsonl`
