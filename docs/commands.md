...
| 会话最前面的原图投喂轮（`ORIGINAL FIGURE SIZE: 36.9mm x 20.1mm` + 图片） | 把原图喂给作图/转换任务 | 归到**本会话第一条任务**的输入附件，不塞给任何工具、也不当"用户消息" |
...
- 旧转录的句柄只有固定前缀 `Tool image output (for your visual review):` → 页面按**顺序**推断到最近一条带 `tool_calls` 的助手消息里、还没被认领的那一次调用，标 `归属：由顺序推断（本轮的 call <id>）`。
- 会话开头的原图轮标 `归属：本会话任务`；推断不出来时标 `归属：未识别` 并让它单独成行（宁可标"没认出来"，也不猜）。
