...
- **调用过程一眼可读**：中栏「轨迹」页是一张**表**（`# / 类型 / 名称 / 一行摘要 / 状态 / 字符 / 耗时`，默认按事件顺序，`23 / 23 步` 计数在右上角），顶部一排类型开关（`全部` / `用户` / `助手` / `思考` / `工具` / `结果` / `元信息` / `用量`，各带条数）可只看某一类；点行首的 `▸` 就地展开该事件的完整输入/输出，**点行号或摘要则跳回「对话」页里对应的那条消息**（对应行闪一下）。摘要列在 `bash`/`compile`/`write_file` 等在**折叠状态下就是一行**：工具名 + `25 → 138 字符`（输入字符数 → 输出字符数）+ `ok`/`error`。
- **页面结构（对照 DSH 的会话界面重做，改的是"组织方式"而不是配色）**：三栏网格 `侧栏 | 中栏 | 详情栏`，两侧都可拖（8px 分隔条，光标 `col-resize`，双击复位）。中栏顶部是**面包屑 + 页签**：`项目 › 会话`（每段最多 220px 截断，当前段加粗），下面一行页签 `对话` / `轨迹`（选中项带 2px 下划线），原本散在工具栏上的开关（自动跟随 / 折叠全部思考 / 仅看工具调用 / 主题）收成小按钮挂在页签行右端。右侧详情栏默认**收起**（0 宽），点会话行尾的 ⓘ 或消息流摘要行的「详情 ›」展开，宽度 300–520px。
...
默认全部展开；过滤时只留下有命中的组并自动展开（命中组标注「过滤命中」）。**展开/折叠状态与三栏宽度都写进 `localStorage`，重开页面也保持**（想回到默认全部展开就清掉 `dsh.sessionview.*`）。
...
中栏「对话」页按 DSH 的消息形态排版：内容列宽 `clamp(680px, 64%, 920px)` 居中；**用户消息是右侧浅蓝气泡**（22px 圆角，最宽 82%），**助手消息左对齐、整列宽、不套气泡也不描边**——正文就是正文。机器内容一律收成**一行折叠行**：思考行 = 折叠箭头 + `思考` + 圆点 + 一行摘要 + `N 字符`，展开后缩进 22px、13px/20px 低对比等宽文本在容器内滚动；工具调用行 = 16px 图标位 + 工具名（按家族着色：`bash`/`compile` 橙、`write_*` 蓝、`read_*`/`view_*` 绿、`grep`/`doc_search`/`list_*` 紫、`submit` 红）+ 圆点 + 一行摘要，右侧给 `25 → 138 字符`（输入 → 输出字符数）与 `ok`/`error`，展开才是「输入 / 输出」两段式卡片（0.5px 边框、代码底色、12px 圆角、每段内滚动上限 260px，各带复制按钮）。工具结果与调用按 `tool_call_id` 配对合进同一张卡片，配不上的结果单独成行不丢。用户消息里的 `images` 渲染成缩略图，点击放大、Esc 关闭；长文本默认只渲染前若干行，展开才进 DOM。每条消息保留序号与所在行号（转录每行都带写入时间戳 `ts`）。消息流最前面只有**一行**极简摘要（消息数 · 输入/输出 · 缓存 · 首字 · 费用 · 提示词快照字符数 + 「详情 ›」），meta 卡片与指标卡都搬进了右侧详情栏。

**系统提示词快照（`t=meta` 原子信息行）**：转录里除消息外还会写入一条 `{"t":"meta",…}` 行，记录**本次运行模型被交代了什么**——`model`、`session_label`、`system_sha`（提示词短哈希）、完整系统提示词 `text`，以及随行的工具定义（`tools[].name/description/parameters`）。它**只记录、永不回放**（`LoadTranscript` 只认 `t=="msg"`，续跑语义与「系统提示词每次运行重新渲染」的原则都不变），也**不计入消息数**。页面把它渲染成**右侧详情栏最下面的「元信息」卡片**（可折叠）：摘要行写明「系统提示词（本次运行快照，不参与回放）」+ 模型 + 会话标签 + 短哈希 + 字符数，展开后等宽显示提示词全文（带「复制」按钮，长提示词在卡片内滚动），下方列出工具名与描述，每个工具的 `parameters` JSON 收在二级折叠里（默认收起）。同一转录有多条 meta 行时（同一会话多次运行、提示词变化）**只显示最后一条**并注明「共 N 条，显示最新」；会话行的悬浮说明里提示「含系统提示词快照」。**老转录（没有 meta 行）照旧正常显示**，不会报错也不显示空卡片。
