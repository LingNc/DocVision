...
- **页面视觉（对照 DSH 的会话界面整体重做）**：扁平、安静、信息优先——面与面之间只用极浅底色差 + 1px 边框分层，不用重描边也不用厚阴影；正文就是正文，**助手的消息不套卡片**（读起来像对话记录而不是一堆盒子），只有机器内容（工具调用/结果、思考、system 提示词、JSON）才进等宽块；"谁在说话"靠一条极窄的角色行（`#序号 · 用户/AI/工具 · 行 N · 思考/工具/图片` 小标签）加缩进区分，**用户消息是唯一带浅底色的一侧**；侧栏三层（项目 → 阶段 → 会话）都是无边框行 + 悬停/选中底色，阶段组头带该阶段在 `progress.json` 里的状态，书级进展单独一行（`images ✓ · style ✓ · convert running`）。深色与浅色是同一套 token 的两份值（浅色默认在 `:root`，深色只在 `html[data-theme="dark"]` 覆盖），切换只改根元素一个属性。
