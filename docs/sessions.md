...
- **模型注册表** `models:`：每个专用 AI（classifier/drawing/style/chapter/convert/checker/verifier）可单独配置 base_url / api_key / model / request_body / stream / thinking / reasoning_effort / tool_stream / **api**（线路协议：`openai` 默认走 OpenAI 兼容 `/chat/completions`；`anthropic` 走 Anthropic Messages API（`base_url` 指到 `/v1`，自动带 `x-api-key`/`anthropic-version` 头）；`responses` 走 OpenAI Responses API（`/v1/responses`，gpt-5/o 系，`store:false` 全量回传、加密思考链回放）。后两条**只走非流式**，thinking/tool_use/图片块与缓存读、stop_reason 全部翻译回内部统一形态，详见 [config.md](config.md)）
