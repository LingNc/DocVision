...
- **模型注册表** `models:`：每个专用 AI（classifier/drawing/style/chapter/convert/checker/verifier）可单独配置 base_url / api_key / model / request_body / stream / thinking / reasoning_effort / tool_stream / **api**（线路协议：`openai` 默认走 OpenAI 兼容 `/chat/completions`；`anthropic` 走 Anthropic Messages API——`base_url` 指到 `/v1`（如 `https://api.anthropic.com/v1`），Claude 及各类兼容网关可用，**只走非流式**，请求自动带 `x-api-key`/`anthropic-version` 头，thinking/tool_use/图片块与 `cache_read_input_tokens`、`stop_reason` 全部翻译回内部统一形态，详见 [config.md](config.md)）
