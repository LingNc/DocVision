...
- **编译预览可细看**：`compile_preview` 的预览留存为会话内 `preview-<n>.png`（含对应 PDF），`view_image {path:"preview.png"}`（最新）或 `preview-<n>.png`（历史）配合裁剪 + zoom 即可细看；zoom 直接从 PDF 高分辨率重渲染（真放大，不是拉伸像素）
