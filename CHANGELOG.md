...
### Fixed
- `sessions --serve` 的监听地址：配置写 `preview.host: "0.0.0.0"` 时横幅显示 `[::]:8849`。根因是 Go 的 `net.Listen("tcp", "0.0.0.0:8849")` 对通配 host 会优先选 IPv6 双栈套接字，内核就回报 `[::]`（配置没错、是地址族挑错了）。现在按**写死的字面 host** 选网络族：`0.0.0.0` → `tcp4`、`::` → `tcp6`，只有 host 留空才交给双栈，loopback 与主机名照旧。
