...
### Added
- **前端迁移重做启动（P5-R1 批，`p5/migration-v2` 分支）**：把转录预览页的前端按「效果与旧页一致、代码组件化」重做。`web/` 新增 Vue 3 + Vite + TS 工程（单文件 IIFE + 单文件 CSS 构建进 `assets/dist/`，迁移期关闭压缩保可调试）；新界面走 `/v2`，旧页 `/` 不动。迁移纪律：样式表整卷沿用旧页（构建后守卫校验 `dist/viewer.css` 与源样式表一致）、组件不写样式；渲染逻辑按块移植、验收 = 结构对照 + 同会话两页截图逐像素 diff + 控制台零报错。本批为脚手架 + Go 侧 `/v2` 路由接线（`sessionview` 新增 `v2.go`，旧页路由与静态导出一律未动）。
