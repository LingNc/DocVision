...
- **T59 空回复不重试**：逐图分析遇到厂商退化空响应（0 choices / 全空无工具调用）原先直接记失败等下轮；现在在重试层原样重发（保前缀缓存），次数共用 `api_max_retries`（默认 3），仍空才算 `[IMG_EMPTY_RESPONSE]`。
- **跨行 `$$…$$` 公式不渲染（预览页）**：行内数学正则逐行扫描，跨行显示块永远匹配不到——块级解析补上 `collectMathBlock`（单行/多行/未闭合三种形态，坏公式回退原文），渲染为居中显示块。
...
- **T58 进度板 running 第五态**：img2text 运行中时，概览进度条出现蓝色脉冲段 + 「进行中 N」计数，方块视图正在处理的书最优先着色——正在跑哪本书一眼可见（判定 = 该图转录在活动窗口内被写过且无结果，仍只从 progress_items 树推导）。
- **中栏「历史 N」页签**：当前会话属于成员数 >1 的 img2text 调用链时，页签行出现第三个标签——该图全链成员列表（来源 chip/标题/时间/状态，当前行高亮、点行切换），切到链外会话自动回落「对话」。
...
- **升级修复会话「消息为空」根因与自愈（T50）**：现场 4 个 mermaid 升级修复会话全部两轮空消息、报 `empty final answer` 失败——查明是厂商网关在高压下返回**退化响应**（`finish=length`、`completion_tokens=0`、无内容/无思考/无工具调用，1.3s 内即返回），不是话术或转录剥离问题（探针复现同结构请求正常）。三层修复：① 会话引擎对退化空响应**原样重发一次**（每次 Run 上限 1 次，仍退化才落既有 nudge/报错路径；有真实输出 token 的空回复仍走 nudge，不误抢）；② mermaid-fix **会话失败或提醒后仍未提交也升级备选模型段**——原先只有编译错误累计到上限才升级，会话失败让备选模型形同虚设；③ 未配置可解析 `fallback_model` 时不再进备选段（零值 ModelConfig 不再空跑）。
- **终端进度行定格保留 + 阶段说明行恢复可见（T28 续）**：紧凑模式下每个流程的实时进度行结束时，把定格行（`[checker:chapter_003] 轮次 5 · 工具调用 8 · 已用 42s`）作为普通行留在滚动区，下一个流程开新行实时刷——跑过的流程终态都看得见，不会被顶掉。根因是 `PrintConsole` 被 quiet 门禁吃掉：它全部 4 个调用点（阶段说明行、进度行定格、最终汇总）都是"必须显示"的行，quiet 本该只压 `Log()` 明细（T11 语义），门禁放错层导致紧凑模式下阶段说明行（含上一批的续跑摘要）从未上屏。
...
- **analyze 错误/警告口径对齐（T36）**：校验未通过/格式不符/空响应（`mermaid_invalid`/`invalid_format`/`empty_response`）从"警告（下轮重试）"改记**失败**——与 T15 起 img2text 进度行、`Skipped invalid` 的 error 级日志一致（分类统计里标注 `失败（跳过、下轮重试）`）；**警告只留给"输出有偏离但已被程序纠正、结果仍可用"**的会话（成功关闭且期间出现 `[WARNING]` 行，如剥离 `[IMG_TYPE:` 前 prose、瞬断自动恢复），不再进错误分布。真实日志（img2text_20260916_130807）复核：empty_response 24 / mermaid_invalid 4 / invalid_format 3 / unknown 1 全部 `[失败]`。

- **Responses / Anthropic Messages API 适配（P4）**：`models.*.<条目>.api: anthropic` 的模型条目改走 Anthropic Messages API（`POST {base_url}/messages`，`base_url` 指到 `/v1`，如 `https://api.anthropic.com/v1`）——Claude 及各类 Anthropic 兼容网关可用。请求自动带 `x-api-key`/`anthropic-version` 头；system 提升为顶层字段，assistant 的思考/文本/工具调用翻译为 thinking/text/tool_use 内容块，工具回执翻译为 tool_result、图片为 base64 source，`user` 路由字段对应 `metadata.user_id`；应答的 `cache_read_input_tokens` 计入前缀缓存命中、`stop_reason` 翻译为 finish_reason、`cache_creation_input_tokens` 计入输入——会话层统计、[cache-probe] 指纹、转录用量照常工作。**两条线路都只走非流式**（流式/实时快照专属能力不适用），重试/4xx 分类与 OpenAI 线路一致。另支持 `api: responses`（OpenAI Responses API，`POST {base_url}/responses`，gpt-5/o 系及兼容网关）：system → `instructions`，助手轮 → output_text + function_call 项、工具回执 → function_call_output、图片 → input_image，固定 `store:false` 全量回传，加密思考链请求 `reasoning.encrypted_content` 并随历史回放（加密思考约半小时级有效期，长间隔续跑可能失效）、`input_tokens_details.cached_tokens` 计入前缀缓存命中。img2text 逐图分析的独立客户端暂仍为 OpenAI 兼容线路。
...
- **实时流式状态（P7）**：会话流式生成期间，对话页消息流末尾出现「思考中 / 输出中」实时卡片（spinner + 新鲜度 + 累积文本尾部 + 闪烁光标）——快照由会话进程在流式回调里节流（~150ms、尾部 16KB）原子写入转录旁的 `<转录>.partial` sidecar，完整消息落盘即删，预览服务（独立进程也能）随增量接口带回；同时轨迹里还没收到回执的调用在摘要行显示绿色「运行中」spinner。转录 append-only 整行语义不变。
- **轨迹点行在右侧详情栏看该步（P9）**：轨迹页点行不再直接跳走，而是选中该步并在右侧详情栏顶部显示「步骤 #N」详情块（类型/名称/输入/输出/思考/用量行/图片，与就地展开同源，带复制与关闭按钮），会话与指标块跟随其后；跳回对话收进行内悬浮显形的 ↳ 小按钮。行身份用新的确定性 `rid`（kind@jump#序号）——jump 是消息行号、一条消息多次调用时会撞，不能当身份。探针同步归一（↳ 按钮/新文案）。
- **灯箱滚轮缩放（P6）**：图片预览支持滚轮缩放（0.15–8 倍、以鼠标位置为锚点）、按住拖动平移、双击复位；拖动过不再触发「点击关闭」，微动后的纯点击行为与原来一致。
- **重画侧栏「打开的文件夹」图标（T19）**：旧 open 图标背板左斜边与前盖左斜边交叉，展开（选中）项目行的图标左侧看着像被糊住；重画为背板左缘垂直直线、前盖斜边整个在背板内侧的画法，两条斜边互不穿插。
- **方块视图编号与完成状态着色（T22）**：章节族会话（转换/核对/样式修复）的方块显示文件名里的章号（`convert_chapter_003` → 3），与逐图会话的图序号同位；方块按会话完成情况着色——运行中绿圈、有过工具回执但从没提交=红（错误终止）、还没开工=暗、提交过（submit 回执含 `SUBMITTED`）=默认样子。完成信号由 Go 侧列表扫描在同一趟流式读取里顺带统计（原始行两次子串检查，零额外解码），悬浮说明补状态语义与检索词。
- **修复 view 类调用展开卡片内输出图片的布局（T18）**：组件化后输出图片经插槽成为 io-section 网格的直接子元素，被自动排进标签列——图片贴最左、把标签列撑到图片宽度、「输出」文字被挤进右侧窄条；现包进 `.io-extra` 钉回内容列，图片在输出正文下方、与文字同列起排，与旧页布局一致（仅 v2，旧页冻结）。
...
- **前端渲染全面 Vue 组件化（P5-R2 批，接续 P5-R1，直接提交 master）**：P5-R1 迁移完成后仍是"命令式 DOM + 薄容器"的时间线、详情栏、轨迹三块全部改成纯 Vue 组件。核心变化：`web/src/legacy/stream.ts` 的 `streamModel()` 把转录行一次扫描成条目模型（助手轮携带调用列表），原来跨节点手术的工具回执挂接与图片归属变成**条目归属记账**——工具回执与图片行记进对应调用的 results/attachments/outImages/previews，由 `ToolCard`/`AssistantMsg` 等组件模板渲染；命令式 DOM 构建函数全部退场，`legacy/` 只留纯数据函数与归属算法。附带修复：**自动跟随补上"手动滚离底部即自动取消"**（旧页行为，组件化初版漏移植）。验收：7 个结构/交互探针 454/454 字段一致、5 场景（对话/轨迹/详情/深色/图片会话）截图 0 像素差、控制台零报错。
- **前端迁移重做（P5-R1 批，`p5/migration-v2` 分支，已完成各块迁移）**：把转录预览页的前端按「效果与旧页一致、代码组件化」重做。`web/` 新增 Vue 3 + Vite + TS 工程（单文件 IIFE + 单文件 CSS 构建进 `assets/dist/`，迁移期关闭压缩保可调试）；新界面走 `/v2`，旧页 `/` 不动。迁移纪律：样式表整卷沿用旧页（构建后守卫校验 `dist/viewer.css` 与源样式表一致）、组件不写样式；渲染逻辑按块移植、验收 = 结构对照 + 同会话两页截图逐像素 diff + 控制台零报错。已迁移：三栏骨架与布局让步链、侧栏三层树（分组/搜索/折叠记忆/轮询签名）、对话时间线（系统消息/思考块/工具卡/图片 FIFO 归属/缩略图预览）、自写 Markdown 渲染器与 JSON/终端/diff 高亮、LaTeX → MathML、轨迹表（类型筛选/就地展开/跳回对话）、详情栏（指标瓦片/每次请求明细/系统提示词快照与工具 schema）、灯箱。逐块验收结果：结构与交互探针全字段一致，明暗两主题截图 0 像素差，控制台零报错。
...
## [v1.5.0-beta.7] - 2026-09-15

本节为补记：标签 `v1.5.0-beta.7`（9027c8e）创建时未同步 CHANGELOG。覆盖范围 = `v1.5.0-beta.6..v1.5.0-beta.7` 的 31 个提交——web 迁移线收尾（`web/components` 合回 master）：P11 CSS Vue 化、P6 灯箱滚轮缩放、P7 实时流式状态（.partial sidecar）、P9 轨迹点行右栏详情、P13 缩略图开关、T18/T19/T20/T21/T22/T23 修复与定性，及验收反馈修复。
...
- **会话预览页：会话名按行内 Markdown 渲染**：名字来自转录文件名或图注，里面的 `` `code` ``／`**粗**`／`_斜_` 现在渲染成对应样式而不是显示标记符号——侧栏会话行、中栏面包屑里的当前会话名、详情栏「会话」那一行三处共用同一个入口（复用消息正文那套渲染器的行内规则，**只走行内**：名字仍是单行、不会出现段落/标题/换行，名字里的 `<`、`&` 依旧只是文本，也依旧只经 DOM API 落树）。短写/截断（图注 → 可读标签 → 前 8 位哈希 + 扩展名）照旧先算好再渲染，搜索仍按**原始文本**匹配（拿 `**`、反引号本身也能搜到），实时轮询的列表签名守卫不受影响（渲染不进签名、不进最小修补）；会话行的悬浮说明改成以**原始**名字开头（标记符号一个不少），`--list` 与终端输出不做渲染。行内 code 沿用 `--code-font`／`--code-line` 那套 token，没有新字号/新颜色。

## [v1.5.0-beta.5] - 2026-09-12

### Added

- **会话预览页：显示单位开关（`token` / `字符`，默认 `token`）**：页签行右端新增一个二选一开关，行内一切按字数统计的地方（工具行 `输入 → 输出`、折叠按钮「展开全文（N 行 / …）」、思考行、轨迹表的计数列与表头、系统提示词快照）都跟着它换口径——**token 一律是本地估算、必带 `≈`；字符是精确计数**。详情栏「指标」里的数字是厂商实测值（`t="usage"` 照抄），不带 `≈`，也不受开关影响。估算器只有一份（`internal/session` 的上下文阈值估算器），由 Go 侧算好随页面数据下发（`Line.est` / `MetaInfo.promptTokenEst` / `ToolSchema.descTokens`），前端不再自算。选择落 `localStorage`（`dsh.sessionview.unit`）。
- **图片 token 按尺寸估算（本地估算）**：`image_url` 的本地成本从"每张固定 1100"改为 `宽×高 / estimate.image_px_per_token`（默认 750，夹在 85–4096；取不到尺寸时退回 `estimate.image_tokens_fallback`，默认 1100），PNG/JPEG/GIF/WebP 头用标准库解析并用内容哈希缓存。**厂商回执的 `prompt_tokens` 一直包含图片 token，不受影响**。新增 `estimate` 配置块（`config_version` 7→8）；详情栏新增「图片 N 张 ≈ …」瓦片，把其中属于图片的部分单列出来对照。
- **会话预览页：看图类调用的缩略图预览行**（用户追加第 2 条）：`view_image`/`view_pdf` 那一行工具调用**下面紧跟**一行缩略图（高 ≤ 108px、宽 ≤ 240px、保持原比例、8px 间距、左对齐、点图进灯箱），**折叠态就可见**——所以它是 `<details>` 的兄弟节点，不塞进卡片。一轮里调用了多次 view 时，这一轮的缩略图**按转录顺序全部并排挂在最后一个 view 行下面**（不是每个 view 行各挂一张），超宽换行；行高与上下间距仍走上面那组 token。
- **会话预览页：精确匹配时图片就是那次调用的输出**（用户追加第 3 条）：句柄写明 `(call <id>)` 时，图片直接渲染在卡片「输出」正文正下方（同一列里、滚动区之外），**不再另立「附件（user 轮）」分区**，卡片只留一行小字 `归属：call <id>（<工具>，精确匹配）`；没匹配上（FIFO 推断 / 未识别）的仍走「附件（user 轮）」分区 + 推断脚注——两种呈现一眼可辨，轨迹页的归属依据照旧如实标注。
- **会话预览页：调用卡片按工具家族着色 + 一行摘要**（用户第 6 条）：工具调用卡片左侧色条与工具名按家族着色（shell/write/view/search/submit），折叠状态下就显示最能说明这次调用的一行摘要（`bash` 命令首行、`read_file`/`compile` 的 `path`、`grep` 的 `pattern`、`doc_search` 的查询词，超 90 字符截断），展开才是完整 JSON；配合原有的「仅看工具调用」开关与 `#序号` 配对的工具结果卡片，顺着调用链看一个会话干了什么不必逐个展开（样式对照 DSH 自己的会话界面）。
- **会话预览页：按项目 + 流程阶段组织，逐图会话按图片身份命名**（用户第 8 条）：侧栏变成三层（项目 → 流程阶段 → 会话），项目组头列出 `progress.json` 里各阶段的当前状态，阶段子组头带会话数与阶段状态（完成打 ✓），可各自展开；逐图（矢量图）会话不再只有哈希文件名——页面用 `doc_index.json` 还原出**图片名 / PDF 页 / 第几张 / 图片类型 / 图注**（行标题用图注、行内小标签、按书内图片顺序排序），搜索框对 `p12`/`第12页`/`#3`/`第3张`/图片名/类型同样生效。数据来源：`SessionInfo` 新增 `stage`/`stageTitle`/`imageName`/`imageOrder`/`page`/`imageType`/`imageCaption`/`projectStages` 字段（静态导出与实时服务同一份结构）。
- **逐图校验（默认关闭）**（用户第 9 条）：`latex.figure_check.enabled` 打开后，每张矢量图提交完成会用**能看图**的模型把"重画图 ↔ 原图"比一遍（`model` 默认 `verifier`，两张图同时进会话）。不一致就把问题清单打回**同一个作图会话**继续修（它的上下文还在，最便宜也最准），最多 `max_rounds` 轮（默认 2：一轮校验 + 一轮修正），仍不合格则记警告照常交付——校验是额外的一张网，不是让整本书失败的新理由。模型跑飞/不交结论一律视为通过（与 checker 同口径）。用法与限制写进 `docs/config.md` 与 `docvision latex --help`。
...
- **`docvision sessions` 默认目录与监听地址改由配置决定**：扫描根默认取 `paths.latex_project`（档位2 取 `paths.latex_output`，与 `docvision latex` 自动启动的预览服务同源），配置缺失或该目录不存在时退回运行命令时所在的目录；监听地址优先级 `--addr` > 新增的 `--port` > 配置 `preview.host`/`preview.port` > 内置默认 `127.0.0.1:8848`（`--addr` 的默认值因此留空，否则配置永远排不上）。启动打印三行——URL、扫到的目录、用的配置文件——每项后面带来源标签（`config paths.latex_project` / `--dir` / `config preview.host/port` / `内置默认`），不再只写一句硬编码的 `127.0.0.1:8848`。
- **会话预览页改成 DSH 的界面组织方式（三栏 + 页签 + 轨迹表 + 详情栏）**：上一批只换了配色，用户要的是**结构**——"和 dsh 一样高效、更具展现力"，于是按 DSH 自己的会话界面重排：
  - **三栏网格** `侧栏 | 中栏 | 详情栏`，两侧各一条 8px 可拖分隔条（`cursor: col-resize`、双击复位），宽度偏好落盘；让步链逐字照搬 DSH（中栏保底 640px，侧栏 264–420px / 折起 56px 轨道，详情栏 300–520px，放不下先压详情栏再整块放弃详情栏），视口 < 1024px 自动折起侧栏。
  - **中栏表头 = 面包屑 + 页签**：`项目 › 会话`（每段最多 220px 截断，当前段加重）下面一行 `对话` / `轨迹`，原先散在工具栏上的开关（自动跟随 / 折叠全部思考 / 仅看工具调用 / 主题）收成小按钮挂在页签行右端。
  - **新增「轨迹」页**：整个会话的事件表（`# / 类型 / 名称 / 摘要 / 状态 / 字符 / 耗时`，右上角 `23 / 23 步`），顶部类型开关（全部/用户/助手/思考/工具/结果/元信息/用量，各带条数）可只看一类；`▸` 就地展开完整输入输出，点行则跳到「对话」页里对应的那条消息并高亮。
  - **右侧详情栏**承载会话信息、指标瓦片、每次请求明细与元信息（系统提示词快照 + 工具清单），默认收起——这些原本挤在消息流里；消息流开头只剩一行极简摘要 + 「详情 ›」。窄窗口下点开详情栏而让步链塞不下时，顺手把侧栏折起来腾地方，而不是"点了没反应"。
  - **消息形态照 DSH**：内容列 `clamp(680px, 64%, 920px)` 居中；用户消息是右侧浅蓝气泡（22px 圆角、最宽 82%），助手消息左对齐整列宽、**不套气泡不描边**；思考与工具调用都是**一行折叠行**（16px 箭头/图标位 + 名称 + 2px 圆点 + 一行摘要 + 右侧字符数），工具行右侧给 `25 → 138 字符 · ok`，展开才是「输入 / 输出」两段式卡片（0.5px 边框、代码底色、12px 圆角、每段内滚动上限 260px）。
  - **侧栏三层改用 DSH 的行词汇**：项目行 34px、阶段行 28px、会话行 32px，统一 `padding: 0 8px` / 圆角 8px / 左侧 16px 固定插槽，**去掉了按层级递增的缩进**；超过 8 条会话的阶段只列前 8 条并给一个 28px 的「更多会话（还有 N 个）」按钮，列表底部一条 24px 渐隐遮罩。
  - **展开状态与三栏宽度改为落盘**（`localStorage` 的 `dsh.sessionview.*`），重开页面保持（此前只记在内存里，重开就回到全部展开）。
- **会话预览页整体重做视觉（对照 DSH 的会话界面）**：改成扁平、安静、信息优先的一层——面与面之间只用极浅底色差 + 1px 边框分层（去掉重描边与厚阴影），助手消息**不再套卡片**而是直接排正文，只有机器内容（工具调用/结果、思考、system 提示词、JSON）进等宽块；"谁在说话"改用一条极窄的角色行 + 缩进区分，用户消息是唯一带浅底色的一侧；工具调用/结果改成无边框行 + 一条按**工具家族**着色的 2px 左轨（跑/写/看/搜/提交）；侧栏三层都是无边框行，阶段组头带该阶段状态、书级进展单独一行；指标卡改成瓦片网格。浅深两套配色都按同一套 token 重写（更深的中性灰阶、更小的字号刻度）。
...
- **img2text 回退到"只出 Mermaid"，不再产出 LaTeX/TikZ**（用户："还把 img2text 调整回之前的那种吧，不要让他产出 latex 的了"）：提示词逐字回到 TikZ 引入（`3c9102e`）之前那一版（类型枚举仍是 Mermaid / Markdown table / LaTeX **公式** / 结构化文本 / 代码块 / 图表描述）——img2text 侧的解析与校验处理不了 LaTeX/TikZ 绘图块，2026-09-12 那次真实跑批 81 张图里 **39 张**因此报废（进度行 `errors: 0, warns: 39`，日志 117 条 `Mermaid validation failed … tikz block 1 failed`）。随之：模板禁提名单（`ForbidMention: tikz / pgfplots / latex code block / latex vector`）+ 第 5 条提示词守卫测试钉住；嵌入不再认 `latex`/`tikz` 绘图块；模型若仍返回绘图块，该图按**无效响应**处理（跳过、下轮重试）；无调用方的 `tools.latex.*` 配置块与 `internal/img2text/tikz.go` 的 TeX 编译校验能力一并删除（矢量图重画是档位1 作图会话的职责，走 `internal/latex/tikz.go`）。
- **校验失败日志说真话**：此前文案把 TikZ 编译失败写成 `Mermaid validation failed`，错误详情又取 TeX 输出的**前 400 字节**（正好是 `This is XeTeX` / `Document Class: standalone` 这类版本横幅），真正的 `! Undefined control sequence.` / `l.NNN` 全被截掉。现在按**实际校验对象**命名并带图片路径，错误详情改为**抽取真错误行**，排除版本横幅与 mmdc 开场白，抽不到才退回**最后** N 行。
- **"无效响应"补齐证据**：`[IMG_TYPE:]` 解析失败、无效响应丢弃、修复轮用尽三处都附**截断后的模型原文 + 总字符数**与**期望格式一行摘要**（此前只有结论，无从判断模型回了什么）。
- **`analyze` 不再把 warning 当失败**（用户问"warning 也算失败吗？"）：进度行本来就分列（`errors: 0, warns: 39`），报告却写成"失败 39、成功率 51.9%"。现在**成功 / 警告（校验未通过或格式不符，已跳过、下轮重试）/ 失败（硬错误）**三类分开统计（`Statistics.Warning`、`WarningRate`、`StatusWarning`），基础统计、按文件摘要、线程明细、CSV 状态列与错误分类表同步分列，进度摘要的"无效条目"注明下轮会自动重试并给出可达完成率。

- **会话预览页：图片归属改成按转录顺序 FIFO 配对**（用户追加第 1 条）：旧规则是"就近往前找最近一轮里还没被认领的那次调用"，真实数据上会把图片挂到 **根本不产图的调用**头上——54 个会话 573 条图片行里，10 条被挂给了同轮的 `bash`/`edit_file`/`grep`/`write_file`/`list_fonts`（例如第 2 行 `write_file` + 4 次 `view_image` 的一轮，4 张图被整体错位一格挂到 `write_file` 与前三张 view 上）。现在按转录顺序维护"会产生图片的调用"队列，每条还没归属的图片行领走队列里**最早那个还没被认领、且排在它前面**的调用（一对一）；`(call <id>)` 句柄仍然精确匹配优先，带正文的图片行仍然是任务自己的图（FIFO 不许抢）。**哪次调用会产图用两条依据判**：回执里有 `Image <路径> … attached.` / `PDF page <文件> … attached.` 证据最硬（失败的 `view_pdf` 回执是 `TOOL ERROR`，真数据里 37 次，它们不进队列），没有回执才退回工具名 `view_image`/`view_pdf` 推定。真数据：537 条工具图片行 ↔ 537 次产图调用正好一对一，`归属：未识别` 0 条、孤立图片行 0 条。
- **会话预览页：带附件/回执的工具行被从消息里拽出来，行间距与回合分隔线跟别的行不一样**（用户报的"有的工具在 html 中的显示间隔不一样"）：带图 user 行归到某次调用后，渲染函数把那张卡片**返回**给了调用方，调用方再 `appendChild` 到 `.stream` 上——卡片于是从它所在的助手消息 `section` 里被**搬走**，成了 `.stream` 的直接子节点（真数据里转换会话 40 行都这样）：同一轮里其它工具行间距 6px、它却是 section 之间的间距，回合虚线分隔也套不上它。现在只登记锚点、不交出节点（并钉进源码断言）。同批把行高/间距统一成**一组 token**：`--row-h: 24px`（所有折叠行，184 行实测唯一值 24）、`--row-gap: 6px`（同一条消息里相邻行，实测唯一值 6）、`--divider-gap: 8px`（回合边界 `.5px dashed` 上下留白，实测节间距唯一值 8），虚线只出现在两个 `section.msg` 之间；折叠态与全展开态下这两组值都不变。
- **会话预览页：带附件的那一段把图片挤成一小条、说明文字被推到右边**（用户报的"内容被限制到很小范围、不是左对齐"）：附件段沿用了「输入 / 输出」的 `label | 内容` 两列网格，归属脚注作为第三个子元素掉到第二行第一列，把左列撑成脚注那么宽——于是说明文字被推到右侧、图片只剩一小条。现在附件段是**单列块**（`attach-section`）：说明文字贴块首左对齐 → 图片各自成行、按原比例左对齐 → **归属脚注放最后**，图片尺寸只走 `--attach-img-w: 520px` × `--attach-img-h: 420px`（原先 160px 的缩略图太小），不再是固定空框。

- **会话预览页：任务提示自带的原图被标成「归属：未识别」**（真实矢量图会话复现）：`source/sessions/vector_<书>__<哈希>__<图>.jsonl` 的第 2 行是 `role:"user"` + 1 张原图 + 任务正文（`Redraw the attached image as TikZ.…`）——**会话任务自己带着原图**，既不是纯图片行，也不以 `Tool image output` 开头。归属逻辑此前只把"不带图的 user 行"当任务行，这种会话压根没有任务行可选，整行掉进 `kind:'none'`，页面标「归属：未识别」并渲染成一条孤立的图片行（真数据 36 个矢量图会话里 35 个各中一条）。现在不以 `Tool image output` 开头的带图 user 轮一律归「本会话任务」：自带任务正文的归到它**自己**那条任务行（照系统消息/任务块的样式渲染，图片作为该任务的附件，附件段不再把任务正文抄第二遍），纯图片行排在任务提示前面的仍走 `pendingTaskImages`（两种都钉了 chromium 用例）。顺带修同一函数里另一处「该归属却落到未识别」：句柄的"还没被认领"计数此前跨轮累计，续跑/重放的转录里同一个 call id 出现两次（样式会话有 133 个重复 id）时，重放段的图片会因"本轮调用已被前面的段认领光"而全部认不出——现在**每轮清零**（真数据里样式会话 33 条未识别 → 0，全部回到对应的 `view_pdf` 卡片）。
- **实时预览每 2 秒把侧栏整个重建，"左边那个展开总是被刷新然后自动展开"**（用户报的真实缺陷）：`--serve` 的轮询无条件调 `renderSessions()`，整个侧栏 DOM 被丢掉重建——手动折叠的项目组/阶段组每次都被还原成展开、滚动位置被弹回、鼠标悬停状态被清掉。现在轮询先算**列表签名**（会话 id 顺序 + 消息数 + 费用 + 请求数 + 项目的 `progress.json` 快照 + 过滤词），签名没变就只做"最小修补"（改相对时间、活跃圆点、用量 chip、悬浮说明与选中高亮），**一个 DOM 节点都不新建/搬动**；签名变了（真多了会话 / 用了新 token / 改了进展）才重建，重建后按落盘的记忆还原各组的展开状态并恢复 `scrollTop`。两处容易写错的细节也一并钉进测试：折叠/溢出的记忆键属于**视图状态**，不许进数据签名（否则点一下组头就让整份列表作废重建）；`mtime`/`live` 也不进签名（它们每 2 秒都可能变，唯一的可见表现是行尾时间与活跃圆点，补丁即可）。另修：会话行的悬浮说明以前挂在用量 chip 上，鼠标移到标题上就看不到；现在整行的 `title` 都带完整口径。
- **静态导出的会话字段被手写白名单丢掉**：`docvision sessions` 的静态页把每个会话的字段逐个列进 `state.sessions`，每加一个字段（`projectLegacy`、`stage`、`projectStages`、`imageOrder`、`cost`…）都要记得补一行，漏掉就在静态页里静默少一块 UI——**侧栏的阶段分组、书级进展、逐图会话的图片名/页码、费用列在静态快照里全都不显示**，而 `--serve` 实时模式正常。改成"整份带走、只减掉每会话的 `lines` 大块"，并把这个写法钉进测试。
...
- **`read_file` 点到目录时把 Go 的原始错误丢给模型**（`read latex_project/…/work: is a directory`，现场 21:17 style-fix 卡了一轮），既没说是哪个引用也没说里面有什么，用户看到的就是"文件不可读取"。现在回报"是目录不是文件"并**列出该目录的真实条目**（复用 `suggestInDir`）；`resolve` 里同类提示也不再让模型"用 bash ls"——checker / style-fix 这些会话根本没有 bash。测试 `TestReadFileOnDirectoryListsEntries`、`TestReadFileOnExplicitSubdirectory`。
