...
- **DocVision（本目录主体）**：PDF→Markdown 自动化工作流（Go，模块 `mineru-tools`，代码在 `go/`；运行目录 `/home/share/samba-share/PDF2MD`，config.yaml 与 docvision 二进制随代码更新）。现状要点（**这里是唯一需要日常维护的部分**；每一批的详细改动、真实缺陷与证据数字在 `docs/agent-batch-history.md`）：
  ...
  - **档位1 全流程**：files → classify → process(images) → style（样式虚拟工作区，产出 cls + manual.md + example.tex）→ chapters（划分）→ convert（逐章并发；每章在**自己的 temp 工作区** `work/temp/conv_<章>/work` 里干活——cls/手册/示例都是副本、插图与 wrapper 已铺好、编译就在这棵树里；提交只交**两个路径**（章节 `.tex` + 分片文件夹），代码只把这两个路径拷进 `work/chapters/`，探针与实验留在 temp；每章产物交 **checker 会话**核对：**只有两个只读文件 + 一个只读文件夹**（章节 md、提交的 .tex、`parts/`），工具仅 `read_file`/`grep`/`submit`，默认 50 轮，问题打回**同一个**转换会话最多 3 轮）→ feedback（定向样式修复，同样跑在 temp 工作区、同样交两个路径）→ assemble（修复会话 + 终审会话）→ 整棵 build 树交付 `out/`。档位2 产物是**零注释**的干净 Markdown（矢量图嵌 SVG）。控制台进度行**只统计本次运行**（断点续传的已完成数只在 `Already done: N | To process: M` 行给出，不混进进度百分比）。
  - **多项目布局**：输出根下每本书一个工作区 `<root>/<项目名>/`（`--project` 显式指定；旧单项目布局自动沿用不迁移）。
  - **AI 会话基础设施** `go/internal/session`：可分离工具、上下文窗口、JSONL 转录（append-only；每行带 `ts` 时间戳，每次 API 请求追加一条 **`t="usage"` 用量行**（token/缓存/耗时/首字延迟/`kind`），`t=meta` 与 `t=usage` 只记录**永不回放**；续跑只回放最后一次压缩检查点之后）、**两级压缩**（先零成本本地裁剪、再 AI 摘要；阈值检查在**每次请求之前**）、工具轮次软限制、前缀缓存亲和（稳定 `user` + `[cache-probe]`）。
