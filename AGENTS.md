...
- **DocVision（本目录主体）**：PDF→Markdown 自动化工作流（Go，模块 `mineru-tools`，代码在 `go/`；运行目录 `/home/share/samba-share/PDF2MD`，config.yaml 与 docvision 二进制随代码更新）。现状要点（**这里是唯一需要日常维护的部分**；每一批的详细改动、真实缺陷与证据数字在 `docs/agent-batch-history.md`）：
  - **命令面**：`docvision latex`（档位1 = 图片矢量化 + 全书 LaTeX；档位2 = 干净 Markdown）、`docvision workflow`、`docvision img2text`、`docvision verify`、`docvision sessions`（转录预览页，**按 DSH 的会话界面组织**：三栏网格 `侧栏 | 中栏 | 详情栏`，两侧 8px 分隔条可拖、宽度与展开状态落盘 `localStorage`，让步链照搬 DSH（中栏保底 640 / 侧栏 264–420 或 56 轨道 / 详情栏 300–520，视口 < 1024 自动折侧栏）；中栏表头是面包屑 + `对话`/`轨迹` 页签，开关收在页签行右端；**「轨迹」页是事件表**（类型筛选 + 就地展开输入输出 + 点行跳回对话）；**右侧详情栏**装会话信息、指标瓦片（输入/输出/思考 tokens、缓存命中率、平均首字、输出速度、平均耗时、跨度）与元信息（系统提示词快照 + 工具清单）；侧栏三层——项目 → 流程阶段 → 会话，统一 DSH 行词汇（34/28/32px、16px 固定插槽、**无递增缩进**）、超出 8 条给「更多会话」、逐图会话按**图片名/PDF 页/书内序号/类型/图注**命名与检索；消息形态照 DSH：用户右侧浅蓝气泡、助手整列宽无卡片、思考与工具调用都是一行折叠行（工具行给 `输入 → 输出 字符数 · ok`）；旧转录无用量行时不显示指标；**实时轮询靠列表签名守卫——签名不变只做文本修补，绝不重建侧栏 DOM**）、`docvision setup`。
  ...
  - **详细批次历史（第一批～第三十三批，原文）** → **`docs/agent-batch-history.md`**。查设计理由、历史事故根因时读它；AGENTS.md 只写现状，不再堆批次流水。
