...
- **DocVision（本目录主体）**：PDF→Markdown 自动化工作流（Go，模块 `mineru-tools`，代码在 `go/`；运行目录 `/home/share/samba-share/PDF2MD`，config.yaml 与 docvision 二进制随代码更新）。现状要点（**这里是唯一需要日常维护的部分**；每一批的详细改动、真实缺陷与证据数字在 `docs/agent-batch-history.md`）：
  - **命令面**：`docvision latex`（档位1 = 图片矢量化 + 全书 LaTeX；档位2 = 干净 Markdown）、`docvision workflow`、`docvision img2text`、`docvision verify`、`docvision sessions`（转录预览页：**侧栏三层——项目 → 流程阶段 → 会话**，项目组头显示 `progress.json` 各阶段状态，阶段子组可展开并带会话数与阶段状态；逐图会话按**图片名/PDF 页/书内序号/类型/图注**命名与检索；中间消息时间线 + 工具调用卡片按工具家族着色并带一行摘要（`bash` 命令首行、`path`、`pattern`，配合「仅看工具调用」开关看调用过程）+ **指标卡片**——输入/输出/思考 tokens、前缀缓存命中率、平均首字延迟、输出速度、平均耗时、会话跨度与每次请求明细；旧转录无用量行时不显示指标）、`docvision setup`。
  ...
  - **详细批次历史（第一批～第三十二批，原文）** → **`docs/agent-batch-history.md`**。查设计理由、历史事故根因时读它；AGENTS.md 只写现状，不再堆批次流水。
