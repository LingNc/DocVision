...
- **发布线纪律**：`v1.5.0-beta.N` 为测试线；含 `-` 的标签由 `.github/workflows/release.yml` 发成 **GitHub 预发布**（同样构建 5 平台产物、只是不标 Latest），正式标签标 Latest——工作流**只由标签推送（或 `gh workflow run`）触发，推 master 不构建**，所以"某版本没有 Release"先查 `git ls-remote --tags origin` 看标签推没推。发布后**不要移动既有标签**；仅在"标签创建当天 + 未推送 + 内容确实是笔误"时才允许 `git tag -f`，并在提交信息与回复里说明旧→新指向。其余情况一律打新标签。
