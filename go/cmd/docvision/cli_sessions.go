...
图片另有一格**本地估算**（「图片 N 张 ≈ …」）：厂商的 prompt_tokens 已经包含图片 token，
这一格只是把其中属于图片的部分按尺寸折算出来对照（规则见配置 estimate 块）；
...
两种模式：
  静态导出（默认）  生成一个自包含 HTML（数据/样式/脚本全部内嵌），图片按相对路径
                    ...
                    "静态快照 · 生成于 …"，不做任何轮询。
  实时服务 --serve  启动本地只读 HTTP 服务（监听地址见上：配置 preview.host/port 或
                    内置默认 127.0.0.1:8848），页面每 2 秒轮询 /api/index，当前会话有新行
