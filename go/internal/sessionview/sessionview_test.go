...
func TestWriteStaticHTML(t *testing.T) {
	...
	// v2 bundle 的稳定字符串（localStorage 前缀）随脚本内联进快照。
	if !strings.Contains(page, "dsh.sessionview.") {
...
func TestStaticHTMLMetaAndGroups(t *testing.T) {
	...
	// 页面资源里的关键标识：主题预置脚本、项目分组容器、meta 卡片、思考滚动区。
	// （主题开关本体是 Vue 渲染的，不在壳/内联脚本字符串之外；行为由浏览器测试把守。）
...
func TestServeAPIMetaAndProject(t *testing.T) {
	...
	// v2 壳：主题预置脚本 + Vue 挂载点（开关本体是 Vue 渲染的，不在壳 HTML 里）。
	for _, marker := range []string{"dsh.sessionview.theme", `data-theme="light"`, `id="app"`} {
