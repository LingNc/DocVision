...
func TestScanImg2TextProgress(t *testing.T) {
	...
	// e5 无结果但转录刚刚被写过 → running（T58）；c3 无任何动静 → pending
	sessDir := filepath.Join(root, "sessions", "书.md")
	os.MkdirAll(sessDir, 0o755)
	os.WriteFile(filepath.Join(sessDir, "images_e5.jpg.jsonl"), []byte("{}\n"), 0o644)
	// c3 什么都没有 → pending
	...
	b := books[0]
	if b.Total != 5 || b.Done != 1 || b.Fixed != 1 || b.Escalated != 1 || b.Running != 1 || b.Pending != 1 {
		t.Fatalf("counts total=%d done=%d fixed=%d esc=%d run=%d pending=%d, want 5/1/1/1/1/1",
			b.Total, b.Done, b.Fixed, b.Escalated, b.Running, b.Pending)
	}
	if b.Items[0].Name != "a1.jpg" || b.Items[0].Status != "done" {
		t.Errorf("item0 = %+v", b.Items[0])
	}
	if b.Items[1].Status != "escalated" || b.Items[2].Status != "pending" || b.Items[3].Status != "fixed" || b.Items[4].Status != "running" {
