...
// 用满 warn_ratio 起提醒剩余次数，超出后只说明"已用尽"——从不拦截，也不劝退。
func TestViewBudgetNoteBoundaries(t *testing.T) {
	...
	if !strings.Contains(got, "21/30") || !strings.Contains(got, "9 left") {
		t.Errorf("第 21 次应提醒 9 次: %q", got)
	}
	if g := viewBudgetNote("view_image on a.png", 30, 30, 0.7); !strings.Contains(g, "0 left") {
...
func TestViewBudgetNoteRounding(t *testing.T) {
	...
	if g := viewBudgetNote("x", 17, 24, 0.7); !strings.Contains(g, "7 left") {
		t.Errorf("第 17 次应提醒 7 次: %q", g)
	}
}
