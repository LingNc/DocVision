...
func semanticChecks(cfg *Config) []string {
	...
	req(strings.TrimSpace(cfg.AI.BaseURL) != "", "ai.base_url 不能为空")
	req(strings.TrimSpace(cfg.AI.APIKey) != "", "ai.api_key 不能为空")
	req(!strings.Contains(cfg.AI.APIKey, "YOUR-"), "ai.api_key 仍是占位符，请填入真实 key")
	req(strings.TrimSpace(cfg.AI.Model) != "", "ai.model 不能为空")
