...
func semanticChecks(cfg *Config) []string {
	...
	req(strings.TrimSpace(cfg.AI.BaseURL) != "", "ai.base_url 不能为空（新配置请在 models.text 中设置 base_url）")
	req(strings.TrimSpace(cfg.AI.APIKey) != "", "ai.api_key 不能为空（新配置请在 models.text 中设置 api_key）")
	req(!strings.Contains(cfg.AI.APIKey, "YOUR-"), "api_key 仍是占位符，请填入真实 key（models.text 或 ai 块）")
	req(strings.TrimSpace(cfg.AI.Model) != "", "ai.model 不能为空（新配置请在 models.text 中设置 model）")
	req(cfg.Options.Concurrency >= 1, "options.concurrency 必须 >= 1")
	req(cfg.Options.APITimeout >= 1, "options.api_timeout 必须 >= 1")
	req(cfg.Options.APIConnectTimeout >= 1, "options.api_connect_timeout 必须 >= 1")
